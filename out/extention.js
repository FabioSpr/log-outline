    ...
    provideDocumentSymbols(document) {
        ...
        for (let i = 0; i < document.lineCount; i++) {
            ...
            if (match) {
                ...
                symbols.push(new vscode.DocumentSymbol(testName, '', vscode.SymbolKind.Namespace, line.range, line.range));
                continue;
            }
            match = sequenceRegex.exec(line.text);
            if (match) {
                const seqName = match[1].trim();
                symbols.push(new vscode.DocumentSymbol(seqName, '', vscode.SymbolKind.Namespace, line.range, line.range));
            }
        }
        return symbols;
    }
